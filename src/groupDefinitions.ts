...
export const groupDefinitions = (ctx: Context, definitions: readonly DefinitionNode[]) => {
  ...
  const schemaDef: undefined | SchemaDefinitionNode = definitions.find((
    d,
  ): d is SchemaDefinitionNode => d.kind === "SchemaDefinition")
  ...
  definitions.map(def => {
    ...
    if (def.kind === "InputObjectTypeDefinition" || def.kind === "InputObjectTypeExtension") {
      return ctx.inputTypes[def.name.value] = (ctx.inputTypes[def.name.value] || []).concat(
        def.fields ?? [],
      )
      ...
      ;(ctx.objectTypes[def.name.value] = ctx.objectTypes[def.name.value] || createObjType())
        .subtypes.push(...def.types?.map(x => x.name.value) ?? [])
      for (const { name } of def.types ?? []) {
        ;(ctx.objectTypes[name.value] = ctx.objectTypes[name.value] ?? createObjType())
          .supertypes.push(def.name.value)
      }
    } else if (
      ...
      const objType = ctx.objectTypes[def.name.value] = ctx.objectTypes[def.name.value]
        ?? createObjType()
      objType.fields.push(...def.fields ?? [])
      objType.supertypes.push(...def.interfaces?.map(i => i.name.value) ?? [])
      for (const { name } of def.interfaces ?? []) {
        ;(ctx.objectTypes[name.value] = ctx.objectTypes[name.value] ?? createObjType())
          .subtypes.push(def.name.value)
      }
    ...
    ctx.typeDirectives[def.name.value] = (ctx.typeDirectives[def.name.value] ?? []).concat(
      def.directives ?? [],
    )
  })
}
