...
export default (ctx: Context) =>
  ...
  `export type $$EnumDirectives = ${stringifyObject(
    Object.entries(ctx.enumTypes).map(([name, values]) =>
      [name, stringifyObject(values.filter(v => v.directives?.length).map(value =>
        [value.name.value, stringifyObject(value.directives?.map(dir =>
          [dir.name.value, stringifyObject(dir.arguments?.map(arg =>
