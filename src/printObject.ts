...
export function printObject(entries: ObjectEntry[], multiline = true) {
  ...
  return `{${sep}${
    entries.map(entry => {
      const [originalKey, optional, value] = entry.length === 2
        ? [entry[0], false, entry[1]] as const
        : entry

      const unquotedKey = originalKey[0] === "\"" && originalKey[originalKey.length - 1] === "\""
        ? JSON.parse(originalKey)
        : originalKey
      const requotedProp = unquotedKey.match(/^(?!\d)[\w$]+$/)
        ? unquotedKey
        : JSON.stringify(unquotedKey)
